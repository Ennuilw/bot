...
async def help_(ctx):
    e=discord.Embed(
        ...
        "`help_` **:** _このコマンド_\n"
        "`additional[add] *(id)` **:** _憤死者リストに追加_\n"
        "`sh` **:** _Jsonファイルの中身を見る_\n"
        "`show` **:** _憤死者リストを表示_\n"
        #"`setting_list_f` **:** _jsonファイルを初期化（Ennui専用）_\n"
    )
...
@bot.command()
async def invite(ctx, id:str = None):
    if not id: id =  "979001395703341096"
    else:
...
async def user(ctx, user:discord.Member=None):
    ...
    e.set_footer(text= f"By: {str(ctx.author)}", url = user.avatar.url)
    await ctx.send(embed=e)
